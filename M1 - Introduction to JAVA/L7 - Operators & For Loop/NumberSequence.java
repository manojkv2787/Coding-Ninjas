...
S into two parts : s1, s2, ..., si and si+1, si+2, ….., sn (0 <= i <= n) in such a way that the first 
...
Sequence made up of only increasing numbers or only decreasing numbers is a valid sequence. So, in 
...
public class NumberSequence {
	public static void main(String[] args) {
		...
		int i;
		for (i = 0; i < n - 1; i++) {
			// System.out.println("Elements compared at position "+ i + " and position "+
			// (i+1) + " : " + arr[i] +" " + arr[i+1]);
			if (arr[i] <= arr[i + 1]) {
		...
		// System.out.println("Starting loop for strictly increasing from from position
		// "+i);
		for (int j = i; j < n - 1; j++) {
			// System.out.println("Elements compared at position "+ j + " and position "+
			// (j+1) + " : " + arr[j] +" " + arr[j+1]);
			if (arr[j] >= arr[j + 1]) {
